/*
The script to fetch the subset of the original data and add links to the node
with no out going links The index is remapped to make it continuous in the
resulting subset of data. Index start from 0.

As for the output file, first line is the number of the included nodes, the
following lines indicating the directed links with the first number as the index
of the source node and the second number as the index of the destination node.
...
Outputs:
    Output files:
    data_input_link:    the directed links with the first number as the index of
the source node and the second number as the index of the destination node.
    data_input_meta:    first line indicating the number of the nodes, the
following lines indicating the node index, number of incoming links, number of
outgoing links.
...
Example:
    >datatrim
    fetch the graph from the original, add links for the nodes with no out going
links and store the result in "./data_input",
...
int main(int argc, char *argv[]) {
  ...
  while ((option = getopt(argc, argv, "b:i:o:n")) != -1) switch (option) {
    case 'b':
  ...
  for (i = 0; i < BOUND; ++i) Ncount += flag[i];
  if (Ncount == 0) {
    printf("Upper limit too small and no nodes are engaged. Try larger upper "
           "bound. \n");
    exit(1);
  }
  ...
  if (b_extend) {
    for (i = 0; i < Ncount; ++i)
      if (num_out_links[i] == 0) {
        ++Noncount;
        for (j = 0; j < BOUND; ++j)
          if (flag[j] >= 0) fprintf(fp_dest, "%d\t%d\n", i, flag[j]);
      }
    free(num_out_links);
  }
  fclose(fp_dest);
  ...
  if (b_extend) {
    ...
    printf("There are %d nodes and %d edges in the sub dataset including the "
           "added edges for the nodes with no outgoing links. \n",
           Ncount, Ecount);
  } else
